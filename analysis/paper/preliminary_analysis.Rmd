...
colores_exp2 <- c(E6 = "#EE7733", E9 = "#CC3311", EG3 = "#047994")
```
...
### Analisis menos guess

* El impacto de POL no es homogeneo. Hay momentos donde AmG es muy alto y esto puede estar asociado a la asimilación de observaciones de satelites multiespectrales. Esto es particularmente notorio en la humedad y temperatura.
* En todos los experimentos AmG es similar, es decir no hay un experimento donde esta diferencia va aumentando a medida que pasan los ciclos de asimilación.
* El impacto en ABI en en algunos periodos (varias horas o dias) mayor a los otros experimentos, es decir, AmG es mayor. Esto es notorio principalmente para V y un poco para Q y U. Si contamos la cantidad de veces que el impacto de ABI es mayor que el impacto en los otros 2 experimentos, esto es así en el 60% de las veces si miramos V y 57%y 54% si miramos Q y U respectivamente.
* No se ve que la magnitud del impacto de ABI aumente con el tiempo. Es en general mayor que en los otros experimentos y esto se nota más en momentos estables. 


```{r}
files <- Sys.glob(here("analysis/data/derived_data_long/*/perfiles_*_*"))

ana <- purrr::map(files[1:3], function(f) {
  meta <- unglue(basename(f), "perfiles_{run}_era5_{exp}_long.csv")
  fread(f) %>% 
    .[, exp := meta[[1]][["exp"]]] 
...
gue <- purrr::map(files[4:6], function(f) {
  meta <- unglue(basename(f), "perfiles_{run}_era5_{exp}_long.csv")
  fread(f) %>% 
    .[, exp := meta[[1]][["exp"]]] 
...
iop <- 
 data.table(ini = c(ymd_hms("20181110140000"),
                   ymd_hms("20181111230000"),
                   ymd_hms("20181117070000"),
                   ymd_hms("20181121140000"),
                   ymd_hms("20181122130000")),
              end = c(ymd_hms("20181110230000"),
                   ymd_hms("20181112070000"),
                   ymd_hms("20181117150000"),
                   ymd_hms("20181121220000"),
                   ymd_hms("20181122210000")),
              name = c("IOP04",
                   "IOP05",
                   "IOP06",
                   "IOP07",
...
ana[gue, on = .(date, lev, variable, exp)] %>% 
  .[, .(diff = mean(abs(ana-guess), na.rm = TRUE)), by = .(variable, exp, date)] %>% 
  .[variable %in% c("T", "QVAPOR", "U", "V")] %>% 
  ggplot(aes(date, diff)) +
  geom_vline(data = iop, aes(xintercept = ini)) +
  geom_vline(data = iop, aes(xintercept = end)) +
  geom_line(aes(color = exp), alpha = 0.8) +
  scale_color_manual(values = c(colores_exp2), labels = c(E9 = "POL", EG3 = "ABI", E6 = "CTRL")) +
...
ana[gue, on = .(date, lev, variable, exp)] %>% 
  .[, .(diff = mean(abs(ana-guess), na.rm = TRUE)), by = .(variable, exp)] %>% 
  .[variable %in% c("T", "QVAPOR", "U", "V")] %>% 
  dcast(date + variable ~ exp, value.var = "diff") %>% 
  # .[, ":="(count_diff = fifelse(EG3 > E6 & EG3 > E9, 1, 0))] %>% 
  .[, .(mean(diff)), by = .(variable)]
    # .[, .(sum(count_diff)/.N), by = .(variable)]
...
```
(ref:fss-abi-fcst-per) FSS at each forecast hour calculated over 1-hour accumulated precipitation for 1 mm (a and d), 5 mm (b and e) and 25 mm (c and f) thresholds, at 10 km (a-c) and 100 km (d-f) scales, for all forecast inicialized from CTRL (orange line), POL (red line), and ABI (turquoise line)., fig.width=6, fig.height=5

```{r fss-abi-fcst-per, fig.cap="(ref:fss-abi-fcst-per)"}

files <- Sys.glob(here("analysis/data/derived_data_long/fcst_det/fss/fss_percentiles_1h_*"))

fss <- purrr::map(files, function(f) {
  
  meta <- unglue(basename(f), "fss_percentiles_1h_fcst_det_{ini_date}_{exp}_d02.csv")
...
fss %>% 
  .[, lead_time := as.numeric(date - ini_date)/3600] %>% 
  .[, .(fss = fss_from_top_bottom(sum(top), sum(bottom))), by =.(window, percentil, exp, lead_time)] %>% 
  
  .[window %in% c(1, 11) & percentil %in% c(0.97, 0.99, 0.9975)] %>%
  .[, ":="(w_label = factor(window, labels = c("10~km", "100~km")))] %>%
  ggplot(aes(lead_time, fss)) +
  # geom_hline(yintercept = 1, color = "darkgray") +
  geom_line(aes(color = exp)) +
  scale_color_manual(values = colores_exp, 
                     labels = c(E6_long = "CTRL", E9_long = "POL",
                                EG3_long = "ABI")) +
  # scale_x_datetime(date_labels = "%d", date_breaks = "1 day", expand = c(0, 0)) +
  facet_grid(w_label ~ percentil, labeller = label_parsed) +
  # facet_grid(window ~ percentil) +
  tagger::tag_facets(position = list(x = 0.05, y = 0.90)) +
  labs(color = NULL, x = "Forecast hour", y = "FSS", linetype = NULL) +
  theme_minimal(base_size = 9) +
  theme(legend.position = "bottom",
...
fss[, .(fss = fss_from_top_bottom(sum(top), sum(bottom))), by = .(window, percentil, exp)] %>% 
  # .[q %in% c(1, 5, 10)] %>% 
  # .[, ":="(q_label = factor(q, labels = c("1~mmh^{-1}", "5~mmh^{-1}", "10~mmh^{-1}", "25~mmh^{-1}", "50~mmh^{-1}")))] %>%
  ggplot(aes(window*10, fss)) +
  geom_line(aes(color = exp)) +
  scale_color_manual(values = c(colores_exp), labels = c(E9_long = "POL", EG3_long = "ABI", E6_long = "CTRL")) +
  facet_wrap(vars(percentil), scales = "free_y", labeller = label_parsed) +
  labs(color = NULL, linetype = NULL, x = "Radius of Influence (km)", y = "FSS") +
  theme_minimal() +
  theme(legend.position = "bottom",
        legend.box = "vertical", 
...
perfiles %>% 
  .[variable == "T" & date %between% ymd_hms(c(20181120180000, 20181123120000))] %>% 
  ggplot(aes(date, lev)) +
  geom_contour_fill(aes(z = ana  - era, fill = stat(level)), 
                    breaks = seq(-3, 3, 0.5)) +
  geom_contour2(aes(z = ana - era), color = "white", size = 0.05,
  ...
  facet_wrap(vars(exp), ncol = 3,  labeller = labeller(exp = c(E6 = "CTRL (SATWND)", E9 = "POL (RAD)", 
                                                              E10 = "POL+ABI", EG3 = "ABI"))) +
  labs(fill = "Temperature (K)",
       x = NULL,
       y = "Pressure (hPa)") +
  ...
  perfiles %>% 
  .[variable == "QVAPOR" & date %between% ymd_hms(c(20181120180000, 20181123120000))] %>% 
  ggplot(aes(date, lev)) +
  geom_contour_fill(aes(z = ana  - era, fill = stat(level)),  
  ...
  perfiles %>% 
  .[variable == "U" & date %between% ymd_hms(c(20181120180000, 20181123120000))] %>% 
  ggplot(aes(date, lev)) +
  geom_contour_fill(aes(z = ana  - era, fill = stat(level)), 
                    breaks = seq(-4, 5, 0.5)) +
  geom_contour2(aes(z = ana - era), color = "white", size = 0.05,
  ...
  scale_y_level(name = "PrePressuresión (hPa)", breaks = c(1000, 850, 750, 500, 300, 200, 100)) +
  scale_date(ini = 20181121000000, end = 20181123110000, break_bin = "12 hours") +
  facet_wrap(vars(exp), ncol = 4,  labeller = labeller(exp = c(E6 = "CTRL (SATWND)", E9 = "POL (RAD)", 
                                                              E10 = "POL+ABI", EG3 = "ABI"))) +
  labs(fill = latex2exp::TeX("U ($m$ $s^{-1}$)"),
       x = NULL,
       y = "Pressure (hPa)") +
  ...
  perfiles %>% 
  .[variable == "V" & date %between% ymd_hms(c(20181120180000, 20181123120000))] %>% 
  ggplot(aes(date, lev)) +
  geom_contour_fill(aes(z = ana  - era, fill = stat(level)), 
                    breaks = seq(-4, 7, 1)) +
  geom_contour2(aes(z = ana - era), color = "white", size = 0.05,
  ...
  scale_y_level(name = "Pressure (hPa)", breaks = c(1000, 850, 750, 500, 300, 200, 100)) +
  scale_date(ini = 20181121000000, end = 20181123110000, break_bin = "12 hours") +
  facet_wrap(vars(exp), ncol = 3, 
             labeller = labeller(exp = c(E6 = "CTRL (SATWND)", E9 = "POL (RAD)", 
                                                              E10 = "POL+ABI", EG3 = "ABI"))) +
  labs(fill = latex2exp::TeX("V ($m$ $s^{-1}$)"),
       x = NULL,
       y = "Pressure (hPa)") +
...
fss <- purrr::map(files, function(f) {
  ...
  rbindlist() %>% 
  separate(exp, into = c("exp", "run"))

files <- Sys.glob("~/datosmunin3/EXP/derived_data/fss_1h_ana_2018112018*")
...
rbind(fss, fss_ana) %>% 
  .[q %in% c(1, 10) & window %in% c(1, 11) & 
      date %between% ymd_hms(c(20181120180000, 20181123120000))] %>% 
  .[, ":="(q_label = factor(q, labels = c("1~mm~h^{-1}", "10~mm~h^{-1}")),
           w_label = factor(window, labels = c("10~km", "100~km")))] %>% 
  ggplot(aes(date, fss)) +
  geom_hline(yintercept = 1, color = "darkgray") +
...
```{r}
files <- Sys.glob("~/datosmunin3/EXP/derived_data/mucape/mcape_ana*20181122120000.nc")

files <- files[str_detect(files, "E6|E9|EG3")]


mcape <- map(files, function(f) {
  
  meta <- unglue(basename(f), "mcape_ana_{exp}_{date}.nc")
  ...
  separate(exp, into = c("exp", "run")) %>% 
  .[, run := fifelse(is.na(run), "case study", run)] %>% 
...
mcape %>% 
  ...
  ggplot(aes(x, y)) +
  geom_contour_fill(aes(z = cape_2d, fill = ..level..), proj = norargentina_lambert) +
  scale_fill_brewer(palette = "YlOrRd", direction = 1, 
                    guide = guide_colorsteps(barwidth = 25,
                                                            ...
                                                            frame.colour = "black")) +
  geom_mapa() +
  facet_grid(exp ~ run, labeller = labeller(exp = c(E6 = "CTRL", E9 = "POL",
                                EG3 = "ABI"))) +
  theme_minimal() +
  theme(legend.position = "bottom") +
...
mcape %>% 
  .[mcape_mcin_lcl_lfc == "mcape"] %>% 
  dcast(x + y + exp ~ run, value.var = "cape_2d") %>% 
  .[, diff := `case study` - long] %>% 
    ggplot(aes(x, y)) +
  geom_contour_fill(aes(z = diff, fill = ..level..), proj = norargentina_lambert) +
  scale_fill_divergent_discretised( guide = guide_colorsteps(barwidth = 25,
                                                            barheight = 0.3,
                                                            title.position = "left", 
                                                            title.vjust = 1,
                                                            frame.colour = "black")) +
  geom_mapa() +
  facet_grid(exp ~ ., labeller = labeller(exp = c(E6 = "CTRL", E9 = "POL",
                                EG3 = "ABI"))) +
  theme_minimal() +
  theme(legend.position = "bottom")
```
...
files <- Sys.glob("~/goes.rad.exp/R/*_fcst_det*area*")
...
```{r}
sal_long <- map(Sys.glob(here("analysis/data/derived_data_long/fcst_det/sal*long*")), fread) %>%
  rbindlist() 

sal_long %>% 
melt(measure.vars = c("s", "a", "l")) %>% 
  # .[ini_date == ymd_hms(20181122120000)] %>%
  ggplot(aes(value)) +
...
```{r}
files <- Sys.glob(here("analysis/data/derived_data_long/fcst_det/fss/fss_fcst_det_tb*"))

fss <- purrr::map(files, fread) %>% 
  rbindlist() %>% 
  # .[, q := rep(c(-30, -60, -70, -78, -84, -85), 5920)] %>% 
  .[, lead_time := as.numeric(date - ini_date)/3600] %>% 
  .[]
...
fss %>% 
  .[ini_date == ymd_hms(20181122120000)] %>% 
  .[, .(fss = fss_from_top_bottom(sum(top), sum(bottom))), by =.(window, q, exp, date)] %>% 
  .[q %in% c(30, 60) & window %in% c(1, 11)] %>%
  # .[, ":="(q_label = factor(q, labels = c("1~mm~h^{-1}", "5~mm~h^{-1}", "25~mm~h^{-1}")),
  #          w_label = factor(window, labels = c("10~km", "100~km")))] %>% 
  ggplot(aes(date, fss)) +
  # geom_hline(yintercept = 1, color = "darkgray") +
...
fss %>% 
  ...
  facet_grid(window~q) +
  tagger::tag_facets(position = list(x = 0.05, y = 0.90)) +
  labs(color = NULL, x = "Lead time", y = "FSS", linetype = NULL) +
  theme_minimal(base_size = 9) +
  theme(legend.position = "bottom",
        panel.background = element_rect(fill = "#fbfbfb", color = NA))
...
fss %>% 
  .[, .(fss = fss_from_top_bottom(sum(top), sum(bottom))), by =.(window, q, exp)] %>% 
  .[q %in% c(30, 60, 70, 78)] %>%
  # .[, ":="(q_label = factor(q, labels = c("1~mm~h^{-1}", "5~mm~h^{-1}", "25~mm~h^{-1}")),
  #          w_label = factor(window, labels = c("10~km", "100~km")))] %>% 
  ggplot(aes(window, fss)) +
  # geom_hline(yintercept = 1, color = "darkgray") +
  geom_line(aes(color = exp)) +
  scale_color_manual(values = colores_exp, 
                     labels = c(E6_long = "CTRL", E9_long = "POL",
...
files <- Sys.glob("~/goes.rad.exp/R/fss_1h_fcst_det_2018112212*d02.csv")
...
rbind(fss, fss_det) %>% 
  ...
  .[q %in% c(1, 5, 25) & window %in% c(1, 11) & exp %in% c("E6", "E9", "EG3")] %>% 
  .[, ":="(q_label = factor(q, labels = c("1~mm~h^{-1}", "5~mm~h^{-1}", "25~mm~h^{-1}")),
           w_label = factor(window, labels = c("10~km", "100~km")))] %>% 
  ggplot(aes(date, fss)) +
  # geom_hline(yintercept = 1, color = "darkgray") +
...
```{r}
sal_short <- map(Sys.glob("~/goes.rad.exp/R/sal*"), fread) %>%
  rbindlist() %>% 
  .[, run := "case study"]

sal_long <- map(Sys.glob("~/sal*long*"), fread) %>%
  rbindlist() %>% 
  .[ini_date == ymd_hms(20181122120000)] %>% 
  separate(exp, into = c("exp", "run"), sep = "_")

rbind(sal_short, sal_long) %>% 
  .[exp == "E6"] %>%
  .[, l_cut := cut(l, breaks = seq(0, 2, 0.25))] %>%
  ggplot(aes(s, a)) +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  # geom_hline(data = sal_stat[exp == "E6_long"], aes(yintercept = median_a), linetype = 2) +
  # geom_vline(data = sal_stat[exp == "E6_long"], aes(xintercept = median_s), linetype = 2) +
  geom_point(aes(color = l_cut, shape = run), size = 3) +
  scale_color_viridis_d(direction = -1, guide = guide_colorsteps(show.limits = TRUE)) +
  scale_shape_manual(values = c("x", "o")) +
  coord_equal(ylim =  c(-2, 2), xlim = c(-2, 2)) +
  labs(x = "S - Structure",
       y = "A - Amplitude",
       color = "L - Location") +
  theme_minimal()

colores <- c("E6" = "#EE7733", "E9" = "#CC3311", "EG3" = "#047994")

rbind(sal_short, sal_long) %>% 
  melt(measure.vars = c("s", "a", "l")) %>% 
  ggplot(aes(value)) +
  geom_density(aes(color = exp, linetype = run)) +
