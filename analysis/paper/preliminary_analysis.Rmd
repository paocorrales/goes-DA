...
```{r sondeos-all, fig.width=7, fig.height=8, fig.cap="(ref:sondeos-iop)"}
...
```{r fss-abi-all, fig.cap="(ref:fss-abi-all)", fig.width=6, fig.height=3}
fss[, .(fss = 1 - sum(top)/sum(bottom)), by =.(window, q, exp)] %>% 
  # .[q %in% c(1, 5, 10)] %>% 
...
```{r fss-abi-fcst, fig.cap="(ref:fss-abi)", fig.width=6, fig.height=5}
...
fss %>% 
  ...
  .[, .(fss = 1 - sum(top)/sum(bottom)), by =.(window, q, exp, lead_time)] %>% 
  
  .[q %in% c(1, 5, 25) & window %in% c(1, 11)] %>% 
